...
def parse_args():
    ...
    parser.add_argument("--module_nr", type=int, required=True, help=f"number of module to be synthesized")
...
#    for module_id in range(args.modules):
    if args.module_nr < 0 or args.module_nr > 5:
        raise RuntimeError(f"invalid module number")
    else:
        module_id = args.module_nr
        module_name = f"module_{args.module_nr}"
